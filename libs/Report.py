...
class Report:
	...
	def __init__(self,p_system_app_name,p_report_name,p_reports_path,p_output_type,p_layout,p_stored_report_path,p_user,p_doc_file_name):
		try:	
			...
			if sec.impersonate(p_user,server_ip,admin_user,admin_password):
				...
				log_error.error(log_id+'impersonate failed for ' + p_user)
			...
			log_info.info(log_id+'initialed report ' + p_report_name + ' for ' + p_user + '....')
		except:
	...
	def check_params(self,p_param_keys):
		try:
			log_info.info(log_id+'send for BI server for get params....')
			...
			if sorted(defined_params) != sorted(p_param_keys):
				log_error.error(log_id+'parameter names that sent are different...')
				self.result = self.result + 'parameter names that sent are different...'
				return False

			log_info.info(log_id+'parameters are the same so it is ok....')
			return True
		except:
			log_error.error(log_id+'Error Occured! for check params')
	...
	def set_params(self,param_name_vals):
		try:
			...
			log_info.info(log_id+'added parameter files....')
			return True
		except:
			log_error.error(log_id+'Error Occured! for set params')
	...
	def run(self,parameters_nam_val):
		#try:
			...
			log_info.info(log_id+'start run report ....')
			...
			# run service
			log_info.info(log_id+'send for BI server....')
			#client = zeep.Client(wsdl=server_ip+self.wsdl)
			...
			log_info.info(log_id+'saved output file....')
			self.result = 'success'
			log_info.info('*****************************************************')		
			return True
		# except:
